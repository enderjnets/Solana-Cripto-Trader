"""DEPRECATED — Jupiter Perpetuals trading client (manual Python implementation).

This module attempted raw Anchor instruction building for Jupiter Perps but
failed with InstructionFallbackNotFound (0x65) due to discriminator/IDL
mismatches. It is superseded by agents.jupiter_perp_cli_wrapper.py which
shells out to the official @jup-ag/cli (actively maintained by Jupiter).

All new code should import from agents.jupiter_perp_adapter.py (which uses
the CLI wrapper under the hood). This file is kept for reference only.
