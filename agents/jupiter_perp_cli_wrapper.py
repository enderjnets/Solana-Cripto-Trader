...
v2.13.1: Added sl_price/tp_price passthrough to CLI --sl/--tp flags.
          Added liq_price capture from open response.
"""
...
def open_position(
    ...
    if sl_price is not None and sl_price > 0:
        args += ["--sl", str(sl_price)]
    if tp_price is not None and tp_price > 0:
        args += ["--tp", str(tp_price)]
    ...
    try:
        data = _run(args)
        if isinstance(data, dict) and data.get("error"):
            return OpenResult(success=False, tx_signature="", entry_price=0, size_usd=0, leverage=0, fee_usd=0, liq_price=0, position_pubkey="", error=data["error"])
        ...
        liq_price = float(data.get("liquidationPriceUsd", data.get("liquidationPrice", 0)))
        ...
        return OpenResult(success=False, tx_signature="", entry_price=0, size_usd=0, leverage=0, fee_usd=0, liq_price=0, position_pubkey="", error=str(e))
