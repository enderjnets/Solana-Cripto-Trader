...
# Config
TARGET_MIN_PCT = 0.03  # 3% mínimo
TARGET_MAX_PCT = 0.05  # 5% máximo
...
def calculate_daily_pnl(portfolio: dict, state: dict) -> dict:
    ...
    if state.get("date") != today:
        state = {
            "date": today,
            "starting_capital": portfolio.get("capital_usd", 500.0),
            "target_reached": False,
            "closed_at": None
        }
    ...
    current_capital = portfolio.get("capital_usd", 500.0)
