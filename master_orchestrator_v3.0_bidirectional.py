...
TAKE_PROFIT_PCT = 3.0  # Take Profit al +3%

# 🔧 V3.1: Drift Protocol Simulation Parameters
...
def get_real_prices() -> Dict:
    """Fetch real-time prices from CoinGecko with indicator data"""
    prices = {}
    try:
        ...
        if resp.status_code == 200:
            ...
            prices_with_indicators = calculate_indicators(prices)
...
def calculate_indicators(prices: Dict) -> Dict:
    """Calculate EMA and RSI for all tokens"""
    result = {}
    ...
    for token in prices.keys():
        ...
        # Simple EMA calculation (simplified - in production, use historical data)
        ema_fast = price * 0.9 + price * 0.1
        ema_slow = price * 0.95 + price * 0.05

        # RSI approximation (0-100)
        if change_24h > 0:
            rsi = 50 + min(abs(change_24h) * 2, 50)
        else:
            rsi = 50 - min(abs(change_24h) * 2, 20)

        result[token] = prices[token]
        result[token]["ema_fast"] = ema_fast
        result[token]["ema_slow"] = ema_slow
        result[token]["rsi"] = rsi
        ...
        for token, data in prices.items():
            ...
            if signal:
                opportunities.append({
                    ...
                    "change_24h": change,
                    "direction": LONG if signal in ["long", "pump"] else SHORT
                })
