...
from monte_carlo_engine import MonteCarloEngine, FellerRiskCalculator, analyze_trading_system

# =============================================================================
# MONTE CARLO ANALYZER (v3.1 Monte Carlo)
# =============================================================================

class MonteCarloAnalyzer:
    """
    Analizador Monte Carlo integrado al sistema de trading.
    
    Características:
    - Análisis automático después de cada N trades
    - Cálculo de Risk of Ruin
    - Distribución de resultados
    - Alertas de riesgo
    """
    
    def __init__(self, db_path: str = None, min_trades_for_analysis: int = 10):
        # Use trades.db if not specified
        if db_path is None:
        ...
        self.last_analysis_file = PROJECT_ROOT / "data" / "mc_last_analysis.json"
        self.last_analysis_file.parent.mkdir(parents=True, exist_ok=True)
    
    def get_closed_trades_pnl(self) -> List[float]:
        """Obtener lista de PnL % de trades cerrados desde paper trading state."""
        try:
            # Try paper trading state first
            paper_state_file = PROJECT_ROOT / "data" / "paper_trading_state.json"
            if paper_state_file.exists():
                ...
                if pnl_list:
                    logger.info(f"📊 Loaded {len(pnl_list)} closed trades from paper trading state")
                    return pnl_list
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT pnl_pct FROM trades WHERE status = 'closed' AND pnl_pct IS NOT NULL AND pnl_pct != 0 ORDER BY created_at"
            )
            pnl_list = [row[0] for row in cursor.fetchall()]
            conn.close()
            return pnl_list
    ...
    def run_analysis(self, initial_balance: float = 10000, num_sims: int = 10000) -> Dict:
        """Ejecutar análisis Monte Carlo completo."""
        trades = self.get_closed_trades_pnl()
        
        if len(trades) < self.min_trades:
            return {
                "status": "insufficient_trades",
                "trades_needed": self.min_trades - len(trades),
                "current_trades": len(trades)
            }
        
        logger.info(f"🎲 Running Monte Carlo analysis with {len(trades)} trades...")
        ...
        # Guardar última fecha de análisis
        self.last_analysis_file.write_text(json.dumps({
    ...
    def get_quick_stats(self) -> Dict:
        """Estadísticas rápidas sin simulación completa."""
        trades = self.get_closed_trades_pnl()
...
def main():
    ...
    parser.add_argument("--mc-stats", action="store_true", help="Monte Carlo: Quick stats")
    parser.add_argument("--mc-analyze", action="store_true", help="Monte Carlo: Full analysis")
    parser.add_argument("--mc-simulations", type=int, default=10000, help="Number of MC simulations")
    parser.add_argument("--mc-balance", type=float, default=10000, help="Initial balance for MC")
    
    args = parser.parse_args()
    
    # Monte Carlo commands (no need to initialize full system)
    if args.mc_stats or args.mc_analyze:
        db_path = PROJECT_ROOT / "data" / "trading.db"
        analyzer = MonteCarloAnalyzer(str(db_path))
        
        if args.mc_stats:
            stats = analyzer.get_quick_stats()
            print("\n" + "="*50)
            print("📊 MONTE CARLO - QUICK STATS")
            print("="*50)
            ...
            print(f"   Mean Return: {stats.get('mean_return', 0):.2f}%")
            print(f"   Std Dev: {stats.get('std_return', 0):.2f}%")
            print("="*50)
            sys.exit(0)
        
        if args.mc_analyze:
            result = analyzer.run_analysis(
                initial_balance=args.mc_balance,
                num_sims=args.mc_simulations
            )
            
            if result.get("status") == "insufficient_trades":
                print(f"❌ Need {result['trades_needed']} more trades (have {result['current_trades']})")
                sys.exit(1)
            ...
            print(f"\n📊 Basic Stats:")
            print(f"   Trades: {bs['total_trades']} | Win Rate: {bs['win_rate']*100:.1f}%")
            print(f"   Profit Factor: {bs['profit_factor']:.2f}")
            
            mc = result['monte_carlo']
            print(f"\n🎲 Monte Carlo ({mc['simulations']:,} simulations):")
            print(f"   Balance P10: ${mc['p10_balance']:,.0f}")
            print(f"   Balance P50: ${mc['p50_balance']:,.0f}")
            print(f"   Balance P95: ${mc['p95_balance']:,.0f}")
            print(f"   Drawdown P95: {mc['p95_drawdown']:.2f}%")
            
            ror = result['risk_of_ruin']
            print(f"\n⚠️  Risk of Ruin: {ror['probability_ruin']:.4f}%")
