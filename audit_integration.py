#!/usr/bin/env python3
"""
Auto-Learning Integration Audit
================================
...
def audit_running_processes():
    ...
    print("🔄 AUDIT: Procesos Corriendo")
    print("="*80)
    
    import subprocess
    
    # Check for old master orchestrator
    try:
        result = subprocess.run(
            ['pgrep', '-f', 'master_orchestrator.py'],
            capture_output=True,
            text=True
        )
        if result.returncode == 0:
            pids = result.stdout.strip().split('\n')
            print(f"  ⚠️  Master Orchestrator antiguo corriendo: {pids}")
            print(f"     Debe detenerse antes de activar nuevo sistema")
            return False
        else:
            print(f"  ✅ Master Orchestrator antiguo detenido")
    except Exception as e:
        print(f"  ❌ Error verificando procesos: {e}")
    
    # Check for auto-learning processes
    try:
        result = subprocess.run(
            ['pgrep', '-f', 'run_auto_learning'],
            capture_output=True,
            text=True
        )
        if result.returncode == 0:
            pids = result.stdout.strip().split('\n')
            print(f"  ✅ Auto-Learning corriendo: {pids}")
        else:
            print(f"  ⚠️  Auto-Learning no corriendo (se iniciará)")
    except Exception as e:
        print(f"  ❌ Error verificando procesos: {e}")
...
def main():
    ...
    results = {
        ...
        "Procesos": audit_running_processes(),
    }
