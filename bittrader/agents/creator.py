...
DATOS ACTUALES: BTC=${btc.get('price_usd',0):,} ({btc.get('change_24h',0):+.1f}% 24h)
TRENDING AHORA: {', '.join(trend)}

PATRONES DE TÍTULOS QUE FUNCIONAN EN NUESTRO CANAL:
{patterns}

FORMATO REQUERIDO (responde exactamente así):
TITULO: [título optimizado, máx 60 chars, sin emojis]
DESCRIPCION: [descripción YouTube 100-150 chars con hashtags]
TAGS: [8-10 tags separados por coma, incluir relevantes del tema]
GUION:
[El guión narrado, 100-140 palabras máximo. Estructura:
- Gancho (primeras 5 palabras que enganchan)
...
PATRONES QUE FUNCIONAN EN NUESTRO CANAL:
{patterns}

FORMATO REQUERIDO (responde exactamente así):
TITULO: [título optimizado, máx 70 chars]
DESCRIPCION: [descripción YouTube completa, 200-300 chars, con hashtags]
TAGS: [10-15 tags separados por coma, incluir relevantes del tema]

GUIÓN COMPLETO:
[Guión narrado completo, 400-700 palabras. Estructura:
HOOK (30s): Enganche poderoso con pregunta o dato impactante
PROBLEMA (45s): Por qué esto importa, el dolor del espectador
EXPLICACION (2-3min): El contenido principal con ejemplos concretos
